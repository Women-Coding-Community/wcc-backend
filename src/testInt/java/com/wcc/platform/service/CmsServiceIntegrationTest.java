...
class CmsServiceIntegrationTest {
  ...
  void testGetLandingPage() throws Exception {
    ...
    assertEquals(
        landingPage.getFullBannerSection().getTitle(),
        result.getFullBannerSection().getTitle());

    assertEquals(
        landingPage.getProgrammes().items().size(),
        result.getProgrammes().items().size());
    assertEquals(
        landingPage.getAnnouncements().items().size(),
        result.getAnnouncements().items().size());
    assertEquals(
        landingPage.getEvents().items().size(),
        result.getEvents().items().size());
    assertEquals(
        landingPage.getFeedbackSection().feedbacks().size(),
        result.getFeedbackSection().feedbacks().size());
    assertEquals(
        landingPage.getPartners().items().size(), result.getPartners().items().size());
  }

  // Helpers to load pages from JSON files, falling back to defaults if parsing fails.
  private FooterSection createFooterTest(String fileName) throws JsonProcessingException {
    String content = FileUtil.readFileAsString(fileName);
    return objectMapper.readValue(content, FooterSection.class);
  }

  private LandingPage createLandingPageTest(String fileName) throws JsonProcessingException {
    String content = FileUtil.readFileAsString(fileName);
