...
/**
 * Service for managing mentee applications to mentors.
 * Handles application submission, status updates, and workflow transitions.
 */
@Slf4j
@Service
...
public class MenteeApplicationService {
    ...
    private static final int MAX_MENTOR_APPLICATIONS = 5;
    ...
    public List<MenteeApplication> submitApplications(
        ...
        log.info("Mentee {} submitted {} applications for cycle {}",
            menteeId, mentorIds.size(), cycleId);
    ...
    public MenteeApplication acceptApplication(
        ...
        final MenteeApplication updated = applicationRepository.updateStatus(
            applicationId,
            ApplicationStatus.MENTOR_ACCEPTED,
            mentorResponse
        );

        log.info("Mentor {} accepted application {} from mentee {}",
            application.getMentorId(), applicationId, application.getMenteeId());

        return updated;
    }

    /**
     * Mentor declines an application.
     * Automatically notifies next priority mentor if available.
     *
     * @param applicationId the application ID
     ...
     */
    @Transactional
    public MenteeApplication declineApplication(
            final Long applicationId,
            final String reason) {
        ...
        final MenteeApplication updated = applicationRepository.updateStatus(
            applicationId,
            ApplicationStatus.MENTOR_DECLINED,
            reason
        );

        log.info("Mentor {} declined application {} from mentee {}",
            application.getMentorId(), applicationId, application.getMenteeId());
     ...
     */
    @Transactional
    public MenteeApplication withdrawApplication(
            final Long applicationId,
            final String reason) {
     ...
     */
    public List<MenteeApplication> getMenteeApplications(
            final Long menteeId,
            final Long cycleId) {
    ...
    private void validateMentorIdsList(final List<Long> mentorIds) {
        ...
        if (mentorIds.size() > MAX_MENTOR_APPLICATIONS) {
            throw new IllegalArgumentException(
                "Cannot apply to more than " + MAX_MENTOR_APPLICATIONS + " mentors");
        }
    }
    ...
    private void notifyNextPriorityMentor(final MenteeApplication declinedApplication) {
        final List<MenteeApplication> allApplications =
            applicationRepository.findByMenteeAndCycleOrderByPriority(
                declinedApplication.getMenteeId(),
                declinedApplication.getCycleId()
            );
        ...
        allApplications.stream()
            ...
            .findFirst()
            .ifPresent(nextApp -> {
                log.info("Next priority mentor {} will be notified for mentee {}",
                    nextApp.getMentorId(), nextApp.getMenteeId());
