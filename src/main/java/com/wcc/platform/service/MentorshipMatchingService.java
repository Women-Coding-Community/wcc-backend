...
/**
 * Service for managing confirmed mentorship matches. Handles match creation, lifecycle management,
 * and cleanup.
 */
@Slf4j
@Service
...
public class MentorshipMatchingService {
  ...
  public MentorshipMatch cancelMatch(
      final Long matchId, final String reason, final String cancelledBy) {
  ...
  private void rejectOtherApplications(
      final Long menteeId, final Long cycleId, final Long acceptedApplicationId) {
    ...
    otherApplications.stream()
        ...
        .filter(
            app ->
                app.getStatus().isPendingMentorAction()
        ...
        .forEach(
            app -> {
              ...
              log.info(
                  "Rejected application {} as mentee {} was matched with another mentor",
                  app.getApplicationId(),
                  menteeId);
            });
  }
