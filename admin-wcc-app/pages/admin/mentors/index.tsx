...
export default function MentorsPage() {
  ...
  const normalize = (resp: MentorsResponse): MentorItem[] => {
    ...
    if ((resp as any)?.content && Array.isArray((resp as any).content))
                  ...
                  {m.skills && (
                    <Stack direction="row" spacing={1} flexWrap="wrap" sx={{ mt: 1 }}>
                      ...
                      {m.skills.languages?.map((l) => (
                        <Chip key={`lang-${m.id}-${l}`} label={l} size="small" color="secondary" />
                      ))}
                    </Stack>
                  )}
                  ...
                  {m.menteeSection?.mentorshipType && m.menteeSection.mentorshipType.length > 0 && (
                    <Stack direction="row" spacing={1} flexWrap="wrap" sx={{ mt: 1 }}>
                      {m.menteeSection.mentorshipType.map((t) => (
                        <Chip key={`type-${m.id}-${t}`} label={t} size="small" color="success" />
                      ))}
                    </Stack>
                  )}
